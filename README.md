...
1. **수집** — 무신사 전 카테고리에서 경쟁브랜드(+ 자사 커버낫) 상품의 랭킹, 가격, 리뷰, 이벤트, 코디스냅을 긁어 누적
...
본 시스템은 B.CAVE *AI 솔루션 도입 및 관리 규정 v1.0*에 따라 IT팀 관할로 운영된다. 무신사 스크래핑은 ToS·robots.txt·rate limit을 엄격히 준수하며, 자사 Snowflake 데이터는 읽기 전용으로만 접근한다. 상세는 `GOVERNANCE.md`.

## Owner

- 시스템 오너: IT팀장 정호철
- 인프라파트: 김은호·정은상·조진우 (Docker·네트워크)
- DT파트: 이현우·도재연·이슬비 (Snowflake·뷰어 운영)
